...
class HomeAdvertisementPagingSource(
    private val apiService: ApiServices,

    ) : PagingSource<Int, AdvertisementResponse>() {
    companion object {
        private const val STARTING_KEY: Int = 1
        private const val PAGE_LIMIT = 40
    }

    override fun getRefreshKey(state: PagingState<Int, AdvertisementResponse>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, AdvertisementResponse> {
        return try {
            val pageNo = params.key ?: STARTING_KEY
            val prevKey = if (pageNo == STARTING_KEY) null else pageNo - 1

            val response =
                apiService.getHomeAdvertisements(page = pageNo, perPage = PAGE_LIMIT)
            val nextKey =
