...
sealed interface NavigationAction {
    ...
    data class Pop(private val popToRouteDefinition: NavRouteDefinition? = null, private val inclusive: Boolean = false) :
        NavigationActionRoute {
        override fun navigate(navController: NavController, resetNavigate: (NavigationAction) -> Unit): Boolean {
...
/** These two will work inside NavHost only*/
fun setInBackStack(navController: NavHostController, key: String, value: Any) {
    navController.previousBackStackEntry?.savedStateHandle?.set(key, value)
}

fun <T> getFromBackStack(
    navController: NavHostController,
    key: String,
    removeAfter: Boolean = true
): T? {
    return navController.currentBackStackEntry?.savedStateHandle?.get<T>(key).also {
        if (removeAfter) {
