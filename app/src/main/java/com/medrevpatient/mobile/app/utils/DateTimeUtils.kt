...
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.EEEE
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.HH
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.MM
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.dd
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.hmma
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.mm
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.ss
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.yy
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.yyyy
import com.medrevpatient.mobile.app.utils.DateTimeUtils.DateFormats.yyyyMMddHHmmss
...
object DateTimeUtils {
    ...
    */
    ...
    fun Long.formatSecondsDuration(): String {
        val hours = this / 3600
    ...
    fun formatDateToUTC(
        ...
        if (sourceString.isNullOrEmpty() || sourceDateFormat.isNullOrEmpty() || targetDateFormat.isNullOrEmpty()) {
            return ""
        }

        return try {
            val sourceFormat = SimpleDateFormat(sourceDateFormat, Locale.US).apply {
                timeZone = TimeZone.getDefault()  // Use device local time zone
            }

            val date = sourceFormat.parse(sourceString) ?: return ""

            // Force time to 12:00 PM (prevents shifting errors)
            val calendar = Calendar.getInstance().apply {
                time = date
                set(Calendar.HOUR_OF_DAY, 12) // 12 PM Noon
            ...
            val targetFormat = SimpleDateFormat(targetDateFormat, Locale.US).apply {
                timeZone = TimeZone.getTimeZone("UTC")  // Convert to UTC
            }

            targetFormat.format(calendar.time)  // Return formatted string in UTC
        } catch (e: ParseException) {
            e.printStackTrace()
            ""
        }
